...
public class ManagerController extends BaseController {
    ...
    public void createProject() {
        ...
        while (true) {
            ...
            if (overlapsWithActive) {
                 System.out.println("Error: The specified application period overlaps with another project you manage. Please enter different dates or manage the visibility/dates of the existing project.");
            } else {
                break;
            }
        }
    ...
    public void editProject() {
        ...
        Date newOpeningDate = getDateInput("Enter new Opening Date (yyyy-MM-dd) [" + DateUtils.formatDate(originalOpening) + "] (leave blank to keep): ", true);
        Date newClosingDate = getDateInput("Enter new Closing Date (yyyy-MM-dd) [" + DateUtils.formatDate(originalClosing) + "] (leave blank to keep): ", true);
    ...
    public void deleteProject() {
        ...
        if (confirm.equals("yes")) {
            ...
            if (projects.remove(projectToDelete)) {
                ...
                ProjectService.saveProjects(projects);
                if (removedApps) ApplicationService.saveApplications(applications);
                if (removedRegs) OfficerRegistrationService.saveOfficerRegistrations(officerRegistrations);
                if (removedEnqs) EnquiryService.saveEnquiries(enquiries);
    ...
    public void manageOfficerRegistrations() {
        ...
        if (pendingRegistrations.isEmpty()) {
         ...
         else rejected.forEach(reg -> System.out.println("- NRIC: " + reg.getOfficerNric() + " | Date: " + DateUtils.formatDate(reg.getRegistrationDate())));
    }
    ...
    public void generateApplicantReport() {
        ...
        List<BTOApplication> bookedApplications = applications.values().stream()
            ...
            .sorted(Comparator.comparing(BTOApplication::getProjectName).thenComparing(BTOApplication::getApplicantNric))
            .collect(Collectors.toList());
