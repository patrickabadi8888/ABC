...
public class OfficerController extends ApplicantController {
    ...
    public OfficerController(Map<String, User> users, List<Project> projects, Map<String, BTOApplication> applications,
    ...
    public void registerForProject() {
        ...
        if (officer.hasActiveApplication()) {
            System.out.println("Error: Cannot register to handle a project while you have an active BTO application ("
                    + officer.getAppliedProjectName() + ", Status: " + officer.getApplicationStatus() + ").");
            return;
        }
        if (officer.hasPendingWithdrawal()) {
            System.out
                    .println("Error: Cannot register to handle a project while you have a pending withdrawal request.");
            return;
        }
        ...
        if (availableProjects.isEmpty()) {
            ...
            System.out
                    .println("- You must not already have a registration (Pending/Approved/Rejected) for the project.");
            System.out
                    .println("- You cannot have an active BTO application (Pending/Successful) or Pending Withdrawal.");
            System.out.println(
                    "- You cannot register if the project's dates overlap with a project you are already handling or have a pending registration for.");
    ...
    public void viewRegistrationStatus() {
        ...
        List<OfficerRegistration> myRegistrations = officerRegistrations.values().stream()
                ...
                .sorted(Comparator.comparing((OfficerRegistration officerR) -> officerR.getRegistrationDate())
                        .reversed())
                .collect(Collectors.toList());
    ...
    public void viewAndReplyToEnquiries() {
        ...
        List<Enquiry> projectEnquiries = enquiries.stream()
                ...
                .sorted(Comparator.comparing((Enquiry enquiry) -> enquiry.getEnquiryDate()).reversed())
                .collect(Collectors.toList());
    ...
    public void manageFlatBooking() {
        ...
        List<BTOApplication> successfulApps = applications.values().stream()
                ...
                .sorted(Comparator.comparing(application -> application.getApplicationDate()))
                .collect(Collectors.toList());
        ...
        if (confirm.equals("yes")) {
            if (!details.decrementAvailableUnits()) {
                System.out.println(
                        "Error: Failed to decrement unit count (possibly became zero just now). Booking cancelled.");
                return;
            }
